...
function EditBlog() {
  ...
  const { data :blogData , loading :blogLoading , error :blogError } = useFetch(
    blogId ? `${getEnv("VITE_API_BASE_URL")}/auth/blog/edit/${blogId}` : null,
    {method : "get", credentials:"include"},
    [blogId]
  )

  useEffect(() => {
  // only run when both blog and categories are available
  if (!blogData?.data || !categoryData?.data?.length) return;
  

  form.reset({
    category: blogData.data.category?._id || '',
    title: blogData.data.title || "",
    slug: blogData.data.slug || "",
    blogContent: blogData.data.blogContent || "",
  });

  if (blogData.data.featuredImage) {
  ...
  const onSubmit = async (values) => {
    try {
      ...
      if (blogImg) {
      formData.append("file", blogImg);   // new upload
    } else {
      // send existing image if no new one selected
      formData.append("featuredImage", blogData?.data?.featuredImage || "");
    }
      ...
      const response = await fetch(
        `${getEnv("VITE_API_BASE_URL")}/auth/blog/update/${blogId}`,
        {
          method: "PUT",
          credentials: "include",
          body: formData,
  ...
  if (blogError) return (
    <div className="flex justify-center items-center min-h-screen">
      <p className="text-red-500">Error loading blog: {blogError.message}</p>
    </div>
  );
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Editor
                          props={{ initialData: field.value ? decode(field.value) : '', onChange: handleEditor }}
                        />
                      </FormControl>
                      <FormMessage className="text-red-500 text-sm mt-1" />
                    </FormItem>
